...
#pragma region Templates components
		...
		T* addComponent(entity_t e, Ts&&... args)
		{
			if (std::is_base_of_v<ComponentUpdate, T>)
			{
				...
				auto p = static_cast<ComponentUpdate*>(c);
			...
			auto p = static_cast<ComponentRender*>(c);
			e->cmpsR_[cId] = p;
...
#pragma region MouseRaycast
		...
		entity_t mouseRaycast()
		{
			...
			auto itMap = ordenRendering.rbegin();
