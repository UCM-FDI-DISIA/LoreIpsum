...
void DragManager::OnLeftClickDown()
{
	...
	if (card != nullptr && draggable && CardOnHand(card) && CardIsOfCurrentPlayer(card))
	{
		...
		initialTransformPosWithOffSet.set(initialMousePos - Vector2D(
			card->getComponent<BoxCollider>()->getRect()->w / 2, card->getComponent<BoxCollider>()->getRect()->h / 2));
	}
}
...
bool DragManager::CardIsOfCurrentPlayer(ecs::entity_t card)
{
	...
	{
		cardOwner = card->getComponent<Transform>()->getParent()->getEntity()->getComponent<HandComponent>()->
		                  getOwner();
	}
...
{
	return card->getComponent<Card>()->getCost() <= mngr_->getHandler(ecs::hdlr::MATCH_MANAGER)->getComponent<
		MatchManager>()->getActualPlayerActualActionPoints();
}
...
void DragManager::putCardOnBoard(ecs::entity_t card, DropDetector* cell)
{
	...
	Players::Owner playerTurn = mngr_->getHandler(ecs::hdlr::MATCH_MANAGER)->getComponent<MatchManager>()->
	                                   getPlayerTurn();
	...
	if (ent != nullptr && ent->hasComponent<TutorialManager>())
	{
		...
		ent->getComponent<TutorialManager>()->tutorialActionEnded(Tutorials::Tutorials::BOARD,
		                                                          Tutorials::Board::PLACE_CARD);
