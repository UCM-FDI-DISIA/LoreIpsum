	...
	else
	{
		for (int i = 0; i < best->_jugada.cartas.size(); i++)
		{
			std::cout << "Carta Indice: " << best->_jugada.cartas[i].indice <<
				" Pos: (" << best->_jugada.cartas[i].pos.getX() << "," << best->_jugada.cartas[i].pos.getY() << ")" <<
				std::endl;
		}
	}
...
)
{
	...
	{
		if (puntosRestantes < 4)
		{
			//si hemos consumido algun punto, cambiar magic number
...
int IA_manager::minimax(int depth, int h, bool isPlayer, State& current_state, State*& best)
{
	...
	for (State& s : all_posible_next_states(current_state, isPlayer))
	{
		...
		if (isPlayer && current > bestValue)
		{
			//si es jugador, maximiza el valor			
			bestValue = current;
			best = new State(s);
		}
		else if (!isPlayer && current < bestValue)
		{
			//si es la IA, lo minimiza
			bestValue = current;
			best = new State(s);
		}
	}
...
void IA_manager::makePlay(const InfoJugada& play) const
{
	...
	for (int i = 0; i < cards.size(); i++)
	{
		...
		if (pos != Vector2D(-1, -1))
		{
			...
			const auto dropDet = boardManager->getCell(pos.getX(), pos.getY())->getEntity()->getComponent<
				DropDetector>();
			...
			const Players::Owner playerTurn = mngr_->getHandler(ecs::hdlr::MATCH_MANAGER)->getComponent<MatchManager>()
			                                       ->getPlayerTurn();
