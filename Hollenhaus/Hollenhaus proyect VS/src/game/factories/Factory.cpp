...
ecs::entity_t Factory::createCard(Vector2D pos, int cost, int value, int id, std::string& sprite, bool unblockable, std::vector<JsonData::CardEffect>& effects)
{
	if (cardFactory == nullptr) {
		throw "no existe cardFactory";
	}

	return cardFactory->createCard(pos,cost,value,id,sprite,unblockable,effects);
