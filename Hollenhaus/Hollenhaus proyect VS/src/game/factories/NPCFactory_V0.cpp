...
ecs::entity_t NPCFactory_V0::createNPC(JsonData::NPCData info, ecs::entity_t parent)
{
	...
	if (info.getSprite() == "whiteRect") { npc->getComponent<SpriteRenderer>()->setOpacity(0); }
	// whiteRect haria de boton
	...
	Vector2D pos{
		GameStateMachine::instance()->getCurrentState()->getLastPaulPos().getX() + info.getPos().getX(),
		info.getPos().getY()
	};
